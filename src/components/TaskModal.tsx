...
function canCurrentUserMove(task: Task, currentUserName: string, currentUserAccess: string): boolean {
  if (currentUserAccess === 'admin' || currentUserAccess === 'superuser') return true
  // user can move if they are the stage owner at the current stage
  const owner = getStageOwner(task, task.status)
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 0, marginBottom: '1.5rem', overflowX: 'auto', paddingBottom: '0.25rem' }}>
      {flow.map((stage, idx) => {
        ...
        const textColor = done ? '#22c55e' : active ? '#a5b4fc' : '#334155'
        return (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '0.3rem', flexShrink: 0 }}>
              <div
                style={{
                  ...
                  outline: active ? '2px solid #6366f122' : 'none',
                }}
              />
              <span style={{ fontSize: '0.55rem', color: textColor, textAlign: 'center', maxWidth: 52, lineHeight: 1.2 }}>
                {stage.label}
              </span>
            </div>
            {idx < flow.length - 1 && (
              <div
                style={{
                  ...
                  height: 1,
                  minWidth: 12,
                  backgroundColor: idx < currentIdx ? '#22c55e44' : '#1e293b',
                  marginBottom: '1rem',
                }}
              />
            )}
...
export function TaskModal() {
  ...
  function handleMove(toStage: StageId) {
    ...
    if (toStage === 'publish') {
      ...
      // Check if current user is the next stage's owner → celebrate
      const nextOwner = getStageOwner(task, toStage)
      if (nextOwner === currentUser.name) {
        playCelebration()
      } else {
        playMove()
      }
    ...
    if (toStage === 'publish') {
      // Trigger celebration overlay via store
      useStore.setState({
        celebration: { taskName: task.name, stageLabel },
        selectedTaskId: null,
      })
    } else {
      selectTask(null)
      ...
      >
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '1.25rem' }}>
          <div style={{ flex: 1, paddingRight: '1rem' }}>
            ...
            <h2 style={{ color: '#f1f5f9', fontSize: '1.1rem', fontWeight: 700, margin: 0, lineHeight: 1.3 }}>
              {task.name}
            </h2>
          </div>
          <button
            onClick={() => selectTask(null)}
            style={{ background: 'none', border: 'none', color: '#475569', cursor: 'pointer', fontSize: '1.2rem', lineHeight: 1, padding: '0.25rem' }}
          >
            ✕
          </button>
        </div>
        ...
        >
          <div>
            <div style={{ fontSize: '0.65rem', color: '#64748b', marginBottom: '0.15rem', textTransform: 'uppercase', letterSpacing: '0.06em' }}>
              Current Stage
            </div>
            <div style={{ color: '#a5b4fc', fontWeight: 600, fontSize: '0.9rem' }}>
              {STAGE_MAP[task.status]?.label ?? task.status}
            </div>
          </div>
          <div style={{ textAlign: 'right' }}>
            <div style={{ fontSize: '0.65rem', color: slaOver ? '#ef4444' : '#64748b', marginBottom: '0.15rem' }}>
              {daysInStage}d / {slaLimit}d SLA
            </div>
            <div
              style={{
                height: 4, width: 80, backgroundColor: 'rgba(255,255,255,0.08)',
            ...
            >
              <div
                style={{
                  height: '100%',
                  width: `${Math.min(daysInStage / slaLimit, 1) * 100}%`,
                  backgroundColor: slaOver ? '#ef4444' : daysInStage / slaLimit > 0.7 ? '#f97316' : '#22c55e',
                  borderRadius: 99,
                }}
              />
            </div>
        ...
        >
          {[
            ...
            >
              <div style={{ fontSize: '0.65rem', color: '#475569', marginBottom: '0.2rem', textTransform: 'uppercase', letterSpacing: '0.05em' }}>
                {label}
              </div>
            ...
            {nextStage && (
              <button
                ...
                style={{
                  flex: 1,
                  padding: '0.65rem',
                  backgroundColor: nextStage === 'publish' ? '#22c55e22' : '#6366f122',
                  border: `1px solid ${nextStage === 'publish' ? '#22c55e44' : '#6366f144'}`,
                  borderRadius: 10,
                  color: nextStage === 'publish' ? '#4ade80' : '#a5b4fc',
                  fontSize: '0.8rem',
                  cursor: 'pointer',
                  fontWeight: 600,
                }}
                onMouseEnter={e => {
                  e.currentTarget.style.backgroundColor = nextStage === 'publish' ? '#22c55e33' : '#6366f133'
                }}
                onMouseLeave={e => {
                  e.currentTarget.style.backgroundColor = nextStage === 'publish' ? '#22c55e22' : '#6366f122'
                }}
              >
        ...
        {(currentUser.access === 'admin') && (
          <div style={{ borderTop: '1px solid rgba(255,255,255,0.06)', paddingTop: '0.75rem', textAlign: 'right' }}>
            {confirmDelete ? (
              <div style={{ display: 'flex', gap: '0.5rem', justifyContent: 'flex-end', alignItems: 'center' }}>
                <span style={{ color: '#ef4444', fontSize: '0.8rem' }}>Sure?</span>
                <button onClick={handleDelete} style={{ padding: '0.35rem 0.75rem', backgroundColor: '#ef444422', border: '1px solid #ef444444', borderRadius: 8, color: '#ef4444', fontSize: '0.78rem', cursor: 'pointer', fontWeight: 600 }}>
                  Delete
                </button>
                <button onClick={() => setConfirmDelete(false)} style={{ padding: '0.35rem 0.75rem', backgroundColor: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 8, color: '#64748b', fontSize: '0.78rem', cursor: 'pointer' }}>
                  Cancel
                </button>
              </div>
            ) : (
              <button
                ...
                style={{ background: 'none', border: 'none', color: '#334155', fontSize: '0.75rem', cursor: 'pointer' }}
              >
                Delete task
              </button>
            )}
