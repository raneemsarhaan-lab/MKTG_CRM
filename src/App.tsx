...
function Sidebar({ view, setView }: { view: View; setView: (v: View) => void }) {
  ...
  return (
    <aside style={{
      width: 56, backgroundColor: '#fff',
      borderRight: '1px solid #e2e8f0',
      display: 'flex', flexDirection: 'column', alignItems: 'center',
      paddingTop: '0.75rem', gap: '0.25rem', flexShrink: 0,
    }}>
      {items.map(item => (
        <button
          key={item.id}
          ...
          style={{
            ...
            backgroundColor: view === item.id ? '#ede9fe' : 'transparent',
            outline: view === item.id ? '1px solid #ddd6fe' : 'none',
            transition: 'all 0.12s',
          }}
          onMouseEnter={e => { if (view !== item.id) e.currentTarget.style.backgroundColor = '#f8fafc' }}
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', backgroundColor: '#f8fafc', overflow: 'hidden' }}>
      ...
      <div style={{ display: 'flex', flex: 1, overflow: 'hidden' }}>
        ...
        <main style={{ flex: 1, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
          ...
          {view === 'team'     && <div style={{ overflowY: 'auto', flex: 1, backgroundColor: '#f8fafc' }}><TeamView /></div>}
          {view === 'capacity' && <div style={{ overflowY: 'auto', flex: 1, backgroundColor: '#f8fafc' }}><CapacityView /></div>}
        </main>
      </div>
